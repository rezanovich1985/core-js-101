...
function doRectanglesOverlap(rect1, rect2) {
  ...
  // Return true if there is overlap in both x-axis and y-axis, otherwise false
...
function isInsideCircle(circle, point) {
  let dist = (point.x - circle.center.x) ** 2;
  dist += (point.y - circle.center.y) ** 2;
  dist = Math.sqrt(dist);
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${start}${a < b ? a : b}, ${a < b ? b : a}${end}`;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (openingBrackets.includes(ch)) {
      ...
      const currBracket = openingBrackets[closingBrackets.indexOf(ch)];
      if (stack.length === 0 || stack[stack.length - 1] !== currBracket) {
        return false;
      }
      stack.pop();
    }
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < directories[0].length; i += 1) {
    const currPath = directories[0][i];
    if (directories.some((directory) => directory[i] !== currPath)) {
      break;
    }
    commonPath += `${currPath}/`;
  }
...
function evaluateTicTacToePosition(position) {
  function evaluateTicTacToeValues(values) {
  ...
  for (let i = 0; i < position.length; i += 1) {
    // const values = position[i];
    const values = [];
    for (let j = 0; j < position[i].length; j += 1) {
      values.push(position[i][j]);
    }

    const res = evaluateTicTacToeValues(values);
    if (res) {
      return res;
    }
  }
  ...
  let values = [];
  for (let i = 0; i < position.length; i += 1) {
    values.push(position[i][i]);
  }
  let res = evaluateTicTacToeValues(values);
  if (res) {
    return res;
  }
  ...
  values = [];
  for (let i = 0; i < position.length; i += 1) {
    values.push(position[i][position.length - i - 1]);
  }
  res = evaluateTicTacToeValues(values);
  if (res) {
    return res;
  }
